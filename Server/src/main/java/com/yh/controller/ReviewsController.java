...
public class ReviewsController {
    ...
    public ReviewsController(ReviewsService reviewsService, JwtUtil jwtUtil) {
    ...
    @Operation(summary = "회고 단건 조회", 
               ...
               parameters = {@Parameter(name = "id", description = "조회할 회고 ID", required = true)})
    @GetMapping("/{id}")
    ...
    @Operation(summary = "회고 수정", 
               ...
               parameters = {@Parameter(name = "id", description = "수정할 회고 ID", required = true)}, 
               requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                   description = "수정할 데이터", 
                   required = true, 
                   content = @Content(schema = @Schema(implementation = Reviews.class))))
    ...
    @Operation(summary = "회고 삭제", 
               ...
               parameters = {@Parameter(name = "id", description = "삭제할 회고 ID", required = true)})
    @DeleteMapping("/{id}")
    ...
    public ResponseEntity<String> convertReviewToImage(@PathVariable long id) {
        try {
            String imageUrl = reviewsService.convertReviewToImage(id);
